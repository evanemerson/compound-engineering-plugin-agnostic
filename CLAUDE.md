...
- Test a workflow `run:` block before pushing: extract it with
  ...
  stubs on `PATH`.
- `CHANGELOG.md` is GENERATED — `scripts/generate-changelog.sh` rebuilds it
  whole from the GitHub Releases, and `.github/workflows/changelog.yml` commits
  it on every release event. Never hand-edit it; fix the release, or the PR
  title it came from.
