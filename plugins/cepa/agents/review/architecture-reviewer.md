...
1. Read `cepa.local.md` from the project root to understand the stack and conventions (especially `service_layer`, `base_templates`, `context_banned_names`).
...
If `service_layer: true` in `cepa.local.md`:
...
Read `base_templates` from `cepa.local.md`:
