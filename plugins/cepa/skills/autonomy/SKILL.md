...
| Which skill owns an *unqualified* citation — `§N` or `§N<letter>` — when two skills define the same anchor (leg 4) | genuinely unresolvable from the citation alone; needs a per-citation owner declaration, not a heuristic. Zero instances today — only this file defines numbered sections at either heading level |
